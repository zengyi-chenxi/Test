		...
		var Elenumber=Request.Elenumber;
		...
		function goUp(thisB) {
			if( flag == true ){//只是评价
				...
				btn1 = thisB;
				$('#Evalalert').css('display','none');//不需要短信了
				submits(4,GetAM());
			}
		}	    
...
//			alert(Keyid);alert(EvalPjs);alert(S1);alert(S2);alert(S3);
//			alert(Content);alert(Reply);alert(photourl);alert(PhotoEvalPath);			
			$.ajax({
				url: contextUrl+"/ExternalInfo.svc/SetElevatorComment",
				timeout : 30000, //超时时间设置，单位毫秒
			    data:JSON.stringify ({//传递参数
					Keyid: Keyid,//商家id
					EvalPj: EvalPjs, //评价等级 0好评 1中评 2差评 3投诉  4报警
					Userid: '',
					S1: S1,//环境评分
					S2: S2,//菜品评分
					S3: S3,//服务评分
				...
				success: function(data) {
					...
					if(json >0) {
						if(EvalPjs == 4){//报警
							...
							alert("评论提交成功!");
                            window.location.href="eleEvaluationlist.html?Keyid="+Keyid+'&Elenumber='+Elenumber+'&Ctext='+Request.Ctext;					
						}
					} else {
						if(EvalPjs == 4){//报警
							alert("报警失败，请重新报警!");
