...
//		OnLoad();
//      var heights = window.screen.height- $('#st-kg').height();		
//		$('#wrapper').css('height',heights); 
	})
	//去评价
	function commentList() {
		window.location.href = 'Evaluationfen.html?Keyid='+ Keyid +'&Ctext='+Request.Ctexts;
	}
	//获取好评度
	function star(Keyid){
		$.ajax({
			url: contextUrl+"/ExternalInfo.svc/EvalGoodAndCountStart",
			data: {
				DeptId: Keyid,
			},
			type: "GET",
...
//				console.log(json);
				if (json[0].好评率 == '' && json[0].菜品 == '' && json[0].环境 == '' && json[0].服务 == '') {
					$("#good_score").html('0%');
					var str='', str1='', str2='';
					for (var i=0; i<5; i++) {
...
//					$("#st-kg").css("display", "none");
	...
	function OnLoads(ul,self,pageNum){
			...
			$.ajax({
				url: contextUrl+"/ExternalInfo.svc/GetCommentList",
...
//					    console.log(json)
	...
	mui("#listnew").on('tap', '.evalslist', function() {
		    ...
		    //打开详情页面
			mui.openWindow({
					id: 'EvaluationDetail.html',
					url: 'EvaluationDetail.html?Fid='+fid+'&laud='+cnum+'&collect='+collects+'&Keyid='+Keyid,	
			});
	});
...
//    初始化加载        EvalPj评价等价 	评价等级 -1:全部评价,0：好评，1：中评 2：差评，3 投诉
//	function OnLoad(){
//		$.ajax({
//			url: contextUrl+"/ExternalInfo.svc/GetCommentList",
//			timeout: 30000, //超时时间设置，单位毫秒
...
//					var str = "";
//					$.each(json, function(index,arr) {
//						str +="<li style='margin-bottom:2rem;' onclick='EvalDetail(\""+json[index].FID+"\",\""+json[index].ReplyCount+"\")'>";
//						str +="<div class='pj-content-top clearfix' Fid='"+json[index].FID+"' collect='"+json[index].ReplyCount+"'>";
//						str +="<div class='pj-top-left pull-left'>";
//						if (json[index].HeadPhoto != null && json[index].HeadPhoto != "" && json[index].HeadPhoto != "AA==") {
//							str +="<div class='pj_toux'><img src=\"data:image/jpeg;base64," + json[index].HeadPhoto + "\" alt=''></div>";
...
//						str +="<div class='clearfix' style='margin:0 4.5rem;'>";
//						if (json[index].LaudCount == 0) {
//							str +="<div class='pj-content-bottom-btm pull-left pl-dzKg' style='margin-right: 6px;'><i class='dz-icon dz-icon"+json[index].FID+"'></i><span class='pl-dzc"+json[index].FID+"'>有用</span></div>";
//						}else{
//							str +="<div class='pj-content-bottom-btm pull-left pl-dzKg' style='margin-right: 6px;'><i class='dz-icon dz-icon"+json[index].FID+"'></i><span class='pl-dzc"+json[index].FID+"'>"+json[index].LaudCount+"</span></div>";
//						}
//						if (json[index].ReplyCount == 0) {
//							str +="<div class='pj-content-bottom-btm pull-left Reply' Fid='"+json[index].FID+"' collect='"+json[index].ReplyCount+"'><i class='pl-icon'></i><span>评论</span></div>";
//						}else{
//							str +="<div class='pj-content-bottom-btm pull-left Reply' Fid='"+json[index].FID+"' collect='"+json[index].ReplyCount+"'><i class='pl-icon'></i><span>"+json[index].ReplyCount+"</span></div>";
//						}											
//						str +="</div>";						
//						str +="</li>";	                            
//					});
//					$("#empInforlist").append(str);
...
//				
//	//评论详情、回复
//	function EvalDetail(Fid, collect){
//		var cnum = Number($(".pl-dzc"+Fid+"").html());
//		window.location.href = 'EvaluationDetail.html?Fid='+Fid+'&laud='+cnum+'&collect='+collect+'&Keyid='+Keyid;
//	}	
